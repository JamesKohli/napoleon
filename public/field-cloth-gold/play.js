...
function on_update() {
	...
	let rs_x = (SCORE_X0 + SCORE_DX * rs_col + SCORE_DXDY * rs_row)
	let rs_y = (SCORE_Y0 + SCORE_DY * rs_row)
	let bs_x = (SCORE_X0 + SCORE_DX * bs_col + SCORE_DXDY * bs_row)
	let bs_y = (SCORE_Y0 + SCORE_DY * bs_row)
	let rs_z = 1
	let bs_z = 1

	if (rs === bs) {
		if (is_action("score", 2)) {
			bs_x -= 10
			bs_y -= 10
			bs_z = 2
		} else {
	...
	ui.blue_score.style.left = bs_x + "px"
	ui.blue_score.style.top = bs_y + "px"
	ui.blue_score.style.zIndex = bs_z
	ui.red_score.style.left = rs_x + "px"
	ui.red_score.style.top = rs_y + "px"
	ui.red_score.style.zIndex = rs_z
